...
class Rewardpoints_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getProductPointsText($product, $noCeil = false, $from_list = false, $customer_group_id = null){
        ...
        if ($points && $product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_GROUPED){
            ...
            if ($from_list){
                return '<p class="j2t-loyalty-points inline-points test 1">'.$img. Mage::helper('rewardpoints')->__("%d reward points", $points) . $this->getEquivalence($points) .$extraPointDetails.'</p>';
            } else {
                return '<p class="j2t-loyalty-points inline-points test 2" style="display:none;">'.$img. Mage::helper('rewardpoints')->__("<span id='j2t-pts'>%d</span> reward points", $points) . $this->getEquivalence($points) . $extraPointDetails.'</p>';
            }
        }
        ...
        if ($points && $product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE && $product->getPriceType() == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED){
            //$return = '<p class="j2t-loyalty-points inline-points test 1">'.$img. Mage::helper('rewardpoints')->__("<span id='j2t-pts'>%d</span> reward points", $points) . $this->getEquivalence($points) . '</p>';
            ...
            if ($from_list && $points_min == $points_max && $points_min == 0){
                ...
                return '<p class="j2t-loyalty-points inline-points test 3">'.$img. Mage::helper('rewardpoints')->__("%d reward points", $points_min) . $this->getEquivalence($points_min) .$extraPointDetails.'</p>';
            } else if ($from_list){ 
                return '<p class="j2t-loyalty-points inline-points test 4">'.$img. Mage::helper('rewardpoints')->__("from %d to %d reward points", $points_min, $points_max) . $this->getEquivalence($points_min, $points_max) .$extraPointDetails.'</p>';
            } else {
                return '<p class="j2t-loyalty-points inline-points test 5" style="display:none;">'.$img. Mage::helper('rewardpoints')->__("<span id='j2t-pts'>%d</span> reward points", $points_min) . $this->getEquivalence($points_min) . $extraPointDetails.'</p>';
            }
        } else if ($points){
            //$details_url = "";
            $extraPointDetails = "";
            ...
            $return = '<p class="j2t-loyalty-points inline-points test 6">'.$img. Mage::helper('rewardpoints')->__("<span id='j2t-pts'>%s</span> reward points", $points) . $this->getEquivalence($points) . $extraPointDetails .'</p>';
            return $return;
        } else if($from_list) {
            ...
            if ($product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_GROUPED && !$attribute_restriction) {
                ...
                if ($catalog_points !== false){
                    ...
                    if (sizeof($product_points)){
                        ...
                        return '<p class="j2t-loyalty-points inline-points test 7">'.$img. Mage::helper('rewardpoints')->__("%d reward points", $points_min) . $this->getEquivalence($points_min) .'</p>';
                    }
                }
            }
		...
		//Fix bundle prices
			...
			$_priceModel  = $product->getPriceModel();
                    ...
                    if ($from_list && $points_min == 0 && $points_max == 0) {
                        return '<p class="j2t-loyalty-points inline-points test 8">'.$img. Mage::helper('rewardpoints')->__("With this product, earned points will depend on product configuration.") .'</p>';
                    } else if ($from_list && $points_min == $points_max){
                        return '<p class="j2t-loyalty-points inline-points test 9">'.$img. Mage::helper('rewardpoints')->__("%d reward points", $points_min) . $this->getEquivalence($points_min) .'</p>';
                    } else if ($from_list){
                        return '<p class="j2t-loyalty-points inline-points test 10">'.$img. Mage::helper('rewardpoints')->__("from %d to %d reward points", $points_min, $points_max) . $this->getEquivalence($points_min, $points_max) .'</p>';
                    } else {
    ...
    public function getPointsOnOrder($cartLoaded = null, $cartQuote = null, $specific_rate = null, $exclude_rules = false, $storeId = false, $money_points = false){
        ...
        // print_r($totals["discount"]->getValue());
        // print_r($totals["subtotal"]->getValue());
