...
class Rewardpoints_Block_Adminhtml_Renderer_Pointstype extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    public function render(Varien_Object $row)
    {
        ...
        if (Rewardpoints_Model_Stats::TYPE_POINTS_REQUIRED == $order_id){
            $current_model = $model->load($row->getRewardpointsAccountId());
            if ($current_model->getQuoteId()){
                ...
                if ($order_model->getIncrementId()){
                    $points_type[Rewardpoints_Model_Stats::TYPE_POINTS_REQUIRED] = Mage::helper('rewardpoints')->__('Required point usage for order #%s (%s)', $order_model->getIncrementId(), Mage::helper('rewardpoints')->__($order_model->getData($status_field)));
                }
            }
        }
