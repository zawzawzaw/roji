...
class Rewardpoints_Block_Points extends Mage_Core_Block_Template
{
    ...
    public function getTypeOfPoint($_point, $referral_id = null)
    {
        ...
        if ($order_id == Rewardpoints_Model_Stats::TYPE_POINTS_REFERRAL_REGISTRATION){
            ...
            if ($model->getName()){
                $toHtml .= '<div class="j2t-in-title">'.$this->__('Referral registration rebates (%s)', $model->getName()).'</div>';
            } else {
                $toHtml .= '<div class="j2t-in-title">'.$this->__('Referral registration rebates').'</div>';
            }
        } else if($referral_id){
            ...
            if ($referrer->getRewardpointsReferralParentId() && Mage::getSingleton('customer/session')->getCustomer() 
                ...
                $toHtml .= '<div class="j2t-in-title">'.$this->__('Referral rebates (%s)',$customer_model->getName()).'</div>';
                ...
                $toHtml .= '<div class="j2t-in-title">'.$this->__('Referral rebates (%s)',$referrer->getRewardpointsReferralEmail()).'</div>';
            }
            ...
            $toHtml .= '<div class="j2t-in-title">'.$this->__('Review rebates').'</div>';
            ...
            $toHtml .= '<div class="j2t-in-title">'.$this->__('Newsletter rebates').'</div>';
        ...
        elseif ($order_id == Rewardpoints_Model_Stats::TYPE_POINTS_GP){
            ...
            if (isset($points_name[$order_id])){
                ...
                $toHtml .= '<div class="j2t-in-title">'.$this->__('Birthday rebates').'</div>';
            }
        }
        elseif ($order_id < 0){
