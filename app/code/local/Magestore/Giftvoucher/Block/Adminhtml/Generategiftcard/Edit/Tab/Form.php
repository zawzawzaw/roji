	...
	if (isset($data['is_generated']) && $data['is_generated'])
        ...
        if ($template && count($template)) {
            $fieldset->addField('giftcard_template_id', 'select', array(
                ...
                'after_element_html' => (isset($data['giftcard_template_image']) && isset($data['giftcard_template_id'])) ? '<script> Event.observe(window, "load", function(){loadImageTemplate(\'' . $data['giftcard_template_id'] . '\',\'' . $data['giftcard_template_image'] . '\');});</script>' : '',
            ));
