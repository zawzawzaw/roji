...
class Magestore_Giftvoucher_Model_Total_Order_Creditmemo_Giftvoucher extends Mage_Sales_Model_Order_Creditmemo_Total_Abstract {
    ...
    public function collect(Mage_Sales_Model_Order_Creditmemo $creditmemo) {
        ...
        if ($baseShippingAmount) {
            $baseTotalDiscountAmountGiftvoucher = $baseTotalDiscountAmountGiftvoucher + $baseShippingAmount * $order->getBaseGiftvoucherDiscountForShipping() / $order->getBaseShippingAmount();
            $totalDiscountAmountGiftvoucher = $totalDiscountAmountGiftvoucher + $order->getShippingAmount() * $baseTotalDiscountAmountGiftvoucher / $order->getBaseShippingAmount();
            $baseTotalDiscountAmountCredit = $baseTotalDiscountAmountCredit + $baseShippingAmount * $order->getBaseGiftcreditDiscountForShipping() / $order->getBaseShippingAmount();
            $totalDiscountAmountCredit = $totalDiscountAmountCredit + $order->getShippingAmount() * $baseTotalDiscountAmountCredit / $order->getBaseShippingAmount();

            $baseTotalGiftvoucherHiddenTax = $baseShippingAmount * $order->getGiftvoucherBaseShippingHiddenTaxAmount() / $order->getBaseShippingAmount();
            $totalGiftvoucherHiddenTax = $order->getGiftvoucherShippingHiddenTaxAmount() * $baseTotalGiftvoucherHiddenTax / $order->getBaseShippingAmount();
            $baseTotalGiftcreditHiddenTax = $baseShippingAmount * $order->getGiftcreditBaseShippingHiddenTaxAmount() / $order->getBaseShippingAmount();
            $totalGiftcreditHiddenTax = $order->getGiftcreditShippingHiddenTaxAmount() * $baseTotalGiftcreditHiddenTax / $order->getBaseShippingAmount();
        }

        if ($this->isLast($creditmemo)) {
            $baseTotalDiscountAmountGiftvoucher = $order->getBaseGiftVoucherDiscount() - $baseGiftvoucherTotalDiscountRefunded;
            $totalDiscountAmountGiftvoucher = $order->getGiftVoucherDiscount() - $totalGiftvoucherDiscountRefunded;
            $baseTotalDiscountAmountCredit = $order->getBaseUseGiftCreditAmount() - $baseGiftcreditTotalDiscountRefunded;
            ...
            $baseTotalGiftvoucherHiddenTax = $order->getGiftvoucherBaseHiddenTaxAmount() - $baseGiftvoucherHiddenTaxRefunded;
            $totalGiftcreditHiddenTax = $order->getGiftcreditHiddenTaxAmount() - $hiddenGiftcreditTaxRefunded;
            $baseTotalGiftcreditHiddenTax = $order->getGiftcreditBaseHiddenTaxAmount() - $baseGiftcreditHiddenTaxRefunded;
        } else {
            foreach ($creditmemo->getAllItems() as $item) {
                ...
                if ($orderItemDiscountGiftvoucher && $orderItemQty) {
                    if (version_compare(Mage::getVersion(), '1.7.0.0', '>=')) {
                        $discount = $creditmemo->roundPrice($orderItemDiscountGiftvoucher / $orderItemQty * $creditmemoItemQty, 'regular', true);
                        $baseDiscount = $creditmemo->roundPrice($baseOrderItemDiscountGiftvoucher / $orderItemQty * $creditmemoItemQty, 'base', true);
                    } else {
                    ...
                    if (version_compare(Mage::getVersion(), '1.7.0.0', '>=')) {
                        $totalGiftvoucherHiddenTax += $creditmemo->roundPrice($orderItemGiftvoucherHiddenTax / $orderItemQty * $creditmemoItemQty, 'regular', true);
                        $baseTotalGiftvoucherHiddenTax += $creditmemo->roundPrice($baseOrderItemGiftvoucherHiddenTax / $orderItemQty * $creditmemoItemQty, 'base', true);
                    } else {
                        $totalGiftvoucherHiddenTax += $orderItemGiftvoucherHiddenTax / $orderItemQty * $creditmemoItemQty;
                        $baseTotalGiftvoucherHiddenTax += $baseOrderItemGiftvoucherHiddenTax / $orderItemQty * $creditmemoItemQty;
                    }
                }
                if ($orderItemDiscountCredit && $orderItemQty) {
                    if (version_compare(Mage::getVersion(), '1.7.0.0', '>=')) {
                        $discount = $creditmemo->roundPrice($orderItemDiscountCredit / $orderItemQty * $creditmemoItemQty, 'regular', true);
                        $baseDiscount = $creditmemo->roundPrice($baseOrderItemDiscountCredit / $orderItemQty * $creditmemoItemQty, 'base', true);
                    } else {
                    ...
                    if (version_compare(Mage::getVersion(), '1.7.0.0', '>=')) {
                        $totalGiftcreditHiddenTax += $creditmemo->roundPrice($orderItemGiftcreditHiddenTax / $orderItemQty * $creditmemoItemQty, 'regular', true);
                        $baseTotalGiftcreditHiddenTax += $creditmemo->roundPrice($baseOrderItemGiftcreditHiddenTax / $orderItemQty * $creditmemoItemQty, 'base', true);
                    } else {
                        $totalGiftcreditHiddenTax += $orderItemGiftcreditHiddenTax / $orderItemQty * $creditmemoItemQty;
                        $baseTotalGiftcreditHiddenTax += $baseOrderItemGiftcreditHiddenTax / $orderItemQty * $creditmemoItemQty;
                    }
                }
            }
            $allowedGiftvoucherBaseHiddenTax = $order->getGiftvoucherHiddenTaxAmount() - $hiddenGiftvoucherTaxRefunded;
            $allowedGiftvoucherHiddenTax = $order->getGiftvoucherBaseHiddenTaxAmount() - $baseGiftvoucherHiddenTaxRefunded;
            $allowedGiftcreditBaseHiddenTax = $order->getGiftcreditHiddenTaxAmount() - $hiddenGiftcreditTaxRefunded;
            $allowedGiftcreditHiddenTax = $order->getGiftcreditBaseHiddenTaxAmount() - $baseGiftcreditHiddenTaxRefunded;
        ...
        $creditmemo->setBaseGrandTotal($creditmemo->getBaseGrandTotal() - $baseTotalDiscountAmountCredit - $baseTotalDiscountAmountGiftvoucher + $totalGiftvoucherHiddenTax + $totalGiftcreditHiddenTax);
        $creditmemo->setGrandTotal($creditmemo->getGrandTotal() - $totalDiscountAmountCredit - $totalDiscountAmountGiftvoucher + $baseTotalGiftvoucherHiddenTax + $baseTotalGiftcreditHiddenTax);
    }
