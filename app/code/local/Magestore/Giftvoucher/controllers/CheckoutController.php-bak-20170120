...
class Magestore_Giftvoucher_CheckoutController extends Mage_Core_Controller_Front_Action {
    ...
    public function addgiftAction() {
        ...
        if ($quote->getCouponCode() && !Mage::helper('giftvoucher')->getGeneralConfig('use_with_coupon') && (!$session->getUseGiftCreditAmount() || !$session->getGiftVoucherDiscount())) {
//            $this->clearGiftcardSession($session);
            ...
            foreach ($addCodes as $code) {
                ...
                if (!$giftVoucher->getId()) {
                    ...
                    if ($max) {
                        $maxErrorMessage = Mage::helper('giftvoucher')->__('You have %d times left to enter gift codes.', $max - count($codes));
                    }
                    ...
                    $result['error'] .= Mage::helper('giftvoucher')->__('Gift code "%s" is no longer available to use.', $code);
                }
            }
    ...
    public function updateAmountAction() {
        ...
        if ($quote->getCouponCode() && !Mage::helper('giftvoucher')->getGeneralConfig('use_with_coupon') && (!$session->getUseGiftCreditAmount() || !$session->getGiftVoucherDiscount())) {
//            $this->clearGiftcardSession($session);
            $result['notice'] = Mage::helper('giftvoucher')->__('Coupon code đã được sử dụng. Mày không thể áp dụng gift code chung với coupon code để discount');
        } else {
