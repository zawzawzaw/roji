...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
        ...
        try {
            ...
            // if order is first order then send out email with discount code

            $orders = Mage::getResourceModel('sales/order_collection')
                ->addFieldToSelect('*')
                ->addFieldToFilter('customer_email', $this->getCustomerEmail());

            if (!$orders->getSize() || $orders->getSize() <= 1) {
                $templateId = 13;
                ...
                $rule = Mage::getModel('salesrule/rule')->load(89);

                if ($rule->getId()) {
                    $generator = Mage::getSingleton('salesrule/coupon_codegenerator', array('length' => 5));
                    $rule->setCouponCodeGenerator($generator);
                    $code = 'NEW'.$rule->getCouponCodeGenerator()->generateCode();

                    // $expirationDate = false;     
                    $days = 30;
                    $expirationDate = time() + $days * 24 * 60 * 60;
     ...
     * @throws Exception
     */
    public function queueFirstNewOrderDiscountCodeEmail($forceMode = false)
    {
    ...
    public function sendNewOrderEmail()
    {
        ...
        // $this->queueFirstNewOrderDiscountCodeEmail(true);
        return $this;
    }
