...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    public function addAction()
    {
        ...
        if($params['isAjax'] == 1){
            ...
            try {
                ...
                // $itemData = $item->getProduct()->getData();
                // $options = $item->getOptions();  
                // $info_buyRequest = '';  

                // $treasure_recipient_name = "";
                // $treasure_message = "";
                // $treasure_ribbon = "";
                ...
                //             $treasure_recipient_name = $unserialized['treasure_recipient_name'];
                //             $treasure_message = $unserialized['treasure_message'];
                //             $treasure_ribbon = $unserialized['treasure_ribbon'];
                //             break;
                //     }
                // }

                // $options['treasure_recipient_name'] = $treasure_recipient_name;
                // $options['treasure_message'] = $treasure_message;
                // $options['treasure_ribbon'] = $treasure_ribbon;
                ...
                // $info_buyRequest_2 = $item->getOptionByCode('info_buyRequest'); 
                ...
                // $info_buyRequest_3 = $item->getCustomOption('info_buyRequest');

                // $item->addOption(new Varien_Object(
                //     array(
                //         'product' => $item->getProduct(),
                //         'code' => 'test',
                //         'value' => '45,46,55'  // 45,46,55
                //     )
                // ));
                ...
                if (!$this->_getSession()->getNoCartRedirect(true)) {
                    if (!$cart->getQuote()->getHasError()){
                        ...
                        // $response['info_buyRequest'] = $info_buyRequest['treasure_ribbon'];
                        // $response['info_buyRequest1'] = $info_buyRequest_2;
                        // $response['info_buyRequest2'] = $info_buyRequest_3;
                    }
                }
    ...
    public function couponPostAction()
    {
        ...
        if(strlen($oldCouponCode) > 0 && $this->getRequest()->getParam('remove') !== "1") {
            // echo $oldCouponCode;
            // echo $this->getRequest()->getParam('remove'); exit();
            $this->_getSession()->addError(
                $this->__('You can only use one discount code at a time.', Mage::helper('core')->escapeHtml($couponCode))
            );
                    ...
                    if ($codeLength) {
                        if ($isCodeLengthValid && $couponCode == $this->_getQuote()->getCouponCode()) {
                            $this->_getSession()->addSuccess(
                                $this->__('Promotional code "%s" was applied.', Mage::helper('core')->escapeHtml($couponCode))
                            );
                        } else {
                            $this->_getSession()->addError(
                                $this->__('Promotional code "%s" is not valid.', Mage::helper('core')->escapeHtml($couponCode))
                            );
                        }
                    } else {
                        $this->_getSession()->addSuccess($this->__('Promotional code was removed.'));
                    }
                }            
