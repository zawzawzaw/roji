...
class Mirasvit_Email_Helper_Html extends Mage_Core_Helper_Abstract
{
    ...
    {
        $typeSource = Mage::getSingleton('email/system_source_chain_type');

        return $this->_getSelect($name, $value, $extra, $typeSource->toArray());
    }

    public function getFrequencyTypeSelect($name, $value = null, $extra = array())
    {
        $frequencyTypeSource = Mage::getSingleton('email/system_source_chain_frequencyType');

        return $this->_getSelect($name, $value, $extra, $frequencyTypeSource->toArray());
    }

    public function getRangeSelect($name, $value = null, $extra = array())
    {
        $rangeSource = Mage::getSingleton('email/system_source_chain_range');

        return $this->_getSelect($name, $value, $extra, $rangeSource->toArray());
    }
