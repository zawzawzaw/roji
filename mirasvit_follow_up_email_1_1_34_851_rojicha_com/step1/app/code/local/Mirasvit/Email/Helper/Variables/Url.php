...
class Mirasvit_Email_Helper_Variables_Url
{
    ...
    /**
     * Retrieve link to subscribe client back.
     *
     * @return string
     */
    public function getSubscribeUrl($parent, $args)
    {
        $query = array();
        if (isset($args[0])) {
        ...
        return $this->_getUrl($parent, 'eml/index/subscribe', $query);
    }
    ...
    public function getReviewUrl($parent, $args)
    {
        ...
        if (isset($args[0]) && ($product = $args[0]) && $product instanceof Mage_Catalog_Model_Product) {
            $query['to'] = base64_encode($parent->getStore()->getUrl('review/product/list/', array('id' => $product->getId(), '_fragment' => 'review-form')));
        }
