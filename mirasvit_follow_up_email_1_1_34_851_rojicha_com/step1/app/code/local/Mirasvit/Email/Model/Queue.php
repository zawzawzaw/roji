 ...
 * @method string setCreatedAt($date)
 */
class Mirasvit_Email_Model_Queue extends Mage_Core_Model_Abstract
{
    ...
    /**
     * @return Mirasvit_Email_Model_Trigger_Chain
     */
    public function getChain()
    {
        if ($this->_chain == null) {
            $this->_chain = Mage::getModel('email/trigger_chain')->load($this->getChainId());
            if (!$this->_chain->getId()) {
                $this->error(Mage::helper('email')->__('Missed Email Chain ID %s', $this->getChainId()));
                Mage::throwException(Mage::helper('email')->__('Trigger Email Chain (ID %s) does not exist anymore.', $this->getChainId()));
            }
        }
    ...
    public function delivery($message = '')
    {
        ...
        Mage::dispatchEvent('email_queue_delivery', array('queue' => $this));
    }
