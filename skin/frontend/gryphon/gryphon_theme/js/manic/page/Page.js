...
manic.page.Page = function(options) {
  ...
  this.options = $j.extend({}, manic.page.Page.DEFAULT, options);
   ...
   * @type {jQuery}
   */
  this.preloader_element = $j('#page-preloader');
  ...
  /**
   * @type {Array.<manic.ui.VideoContainer>}
   */
  this.manic_video_array = [];
  ...
  /**
   * @type {Array.<manic.ui.HoverItem>}
   */
  this.hover_item_array = [];
  ...
  this.mobile_header_spacer_element = $j('#mobile-header-spacer');
  this.mobile_header_element = $j('#mobile-header');
  this.desktop_header_spacer_element = $j('#desktop-header-spacer');
...
manic.page.Page.prototype.create_scrolltarget = function() {
  ...
  var arr = $j('.scroll-target');

  for (var i = 0, l = arr.length; i < l; i++) {
    scroll_target = $j(arr[i]);
...
manic.page.Page.prototype.create_image_container = function() {
  ...
  // var arr = $j('.manic-image-container').not('.not-default-version').not('.instantiated-version');
  var arr = $j('.manic-image-container').not('.manic-image-container.not-default-version').not('.manic-image-container.instantiated-version');
...
manic.page.Page.prototype.create_video_container = function(){
  ...
  var arr = $j('.manic-video-container').not('.not-default-version');
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    ...
    this.manic_video_array[this.manic_video_array.length] = video_container;
  }
...
manic.page.Page.prototype.create_expand_container = function(){
  var arr = $j('.manic-expand-container');
  var item = null;
...
manic.page.Page.prototype.create_hover_item = function() {
  var arr = $j('.manic-hover-item');
  var item = null;
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    item = $j(arr[i]);
    hover_item = new manic.ui.HoverItem({}, item);
    this.hover_item_array[this.hover_item_array.length] = hover_item;
  }
};

manic.page.Page.prototype.create_form_check = function() {
  var arr = $j('.simple-form-check');
  var form_check = null;
  var item = null;
  var item_id = '';
...
manic.page.Page.prototype.create_scroll_down_cta = function(){
  $j('.scroll-down-cta').click(function(event){
...
manic.page.Page.prototype.update_page_layout = function(){
  ...
  this.update_manic_video_containers();
...
manic.page.Page.prototype.update_manic_video_containers = function(){
  /**
   * @type {manic.ui.VideoContainer}
   */
  var item = null;
  
  for (var i = 0, l=this.manic_video_array.length; i < l; i++) {
    item = this.manic_video_array[i];
    item.update_layout();
  }
};
...
manic.page.Page.prototype.scroll_to_after_the_fold = function(){
  ...
  // var target_y = this.window_height - $j('#desktop-header .bottom-header').height();
