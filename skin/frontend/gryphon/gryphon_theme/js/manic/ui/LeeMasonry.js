...
manic.ui.LeeMasonry = function(options, element) {
  ...
  this.options = $j.extend({}, manic.ui.LeeMasonry.DEFAULT, options);
  ...
  */
  ...
  $j(window).resize(this.on_window_resize.bind(this));
  ////this.update_layout();
  //this.update_layout();
...
manic.ui.LeeMasonry.prototype.update_layout = function() {
  ...
  /*
  this.element_width = $j(window).width();
  this.element_height = $j(window).height();
  */
  ...
  */
  ...
  for (var i = 0; i < (this.row_num-1); i++) {
    var line = $j('<div class="horizontal-line"></div>');
    line.css({
...
manic.ui.LeeMasonryItem = function(options, element) {
  ...
  goog.events.EventTarget.call(this);
  this.options = $j.extend({}, manic.ui.LeeMasonryItem.DEFAULT, options);
  this.element = element;
