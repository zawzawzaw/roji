...
manic.ui.PanelContainer = function(options, element) {
  ...
  this.options = $j.extend({}, manic.ui.PanelContainer.DEFAULT, options);
...
manic.ui.PanelContainer.prototype.create_panels_from_array = function(array_param) {
  ...
  var fragment = $j(document.createDocumentFragment());
  var panel_item = null;
  for (var i = 0, l = arr.length; i < l; i++) {
    panel_item = $j('<div class="panel"></div>');
    panel_item.append($j('' + arr[i]));
...
manic.ui.PanelContainer.prototype.create_panels_from_array_with_inner = function(array_param) {
  ...
  for (var i = 0, l = arr.length; i < l; i++) {
    panel_item = $j('<div class="panel"></div>');
    panel_inner = $j('<div class="panel-inner"></div>');

    panel_inner.append($j('' + arr[i]));
