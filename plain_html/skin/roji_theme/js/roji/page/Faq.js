...
roji.page.Faq = function(options, element) {
  ...
  this.options = $.extend(this.options, roji.page.Faq.DEFAULT, options);


  // variables for update_faq_bg_width
  this.sidebar_bg = $('#page-faq-content-section-bg .sidebar-bg');
  this.content_bg = $('#page-faq-content-section-bg .content-bg');
  this.sidebar_width = $('#page-faq-sidebar-width');
  this.content_width = $('#page-faq-content-width');
  
  
  // variables for update_faq_bg_height
  this.desktop_title_section = $('#page-faq-title-section');
  this.section_bg = $('#page-faq-content-section-bg');
...
roji.page.Faq.prototype.show_faq_content_item = function(str_param) {
  ...
  if (manic.IS_MOBILE == false) {
    ...
    // desktop version
    var target_content_item = $('.page-faq-content-item[data-value="' + str_param + '"]');
    var target_sidebar_item = $('#page-faq-sidebar nav ul li[data-value="' + str_param + '"]');
    ...
    if (target_content_item.length != 0) {
      ...
      if (target_content_item.hasClass('open-version')) {
        ...
        $('.page-faq-content-item').removeClass('open-version');
        target_content_item.addClass('open-version');

        $('#page-faq-sidebar nav ul li').removeClass('selected');
